...
Sincronizado is designed to bridge the gap between **local developer experience** (low latency, familiar tools, custom configuration) and **remote execution power** (heavy AI models, specific OS environments, massive compute).
...
graph TB
  subgraph Local["Local Machine"]
    Editor["Your Editor (VS Code / JetBrains / Neovim)"]
    Sinc["sinc CLI"]
...
## Why not VS Code Remote - SSH?

VS Code Remote - SSH is fantastic, but it has a few limitations that Sincronizado addresses:

1.  **Heavy Remote Requirements**: VS Code installs a substantial "server" component on the remote. On smaller VPS instances, this can eat up RAM that you'd rather save for your AI agent.
2.  **Latency Sensitivity**: If the connection drops, your editor stops working. With Sincronizado, your editor is local. You can keep typing, scrolling, and searching files even if the internet goes out. Changes will just sync when you reconnect.
3.  **Editor Agnostic**: Sincronizado works with *any* editor. Neovim, IntelliJ, Zed, Sublime Text—as long as they edit files on your disk, they work.
