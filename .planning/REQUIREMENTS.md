...
- [ ] **SYNC-10**: `sinc push` and `sinc pull` show a clear "about to overwrite" summary (direction, session id, local path, remote path)

### safety

- [ ] **SAFE-01**: `sinc push` and `sinc pull` require explicit confirmation unless `--yes` is provided
- [ ] **SAFE-02**: `sinc push` and `sinc pull` fail fast with a clear error when no sync session exists for the current directory (with suggestion to run `sinc` first)
...
*Last updated: 2026-02-04 - milestone v1.1 initialized*
